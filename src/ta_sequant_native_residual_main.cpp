...
// code.
//
// Emits the same TAStageResult/CSV schema as ta_benchmark_main.cpp
// (shared via ta_stage.h)
...
int main(int argc, char** argv) {
  ...
  // Allow comparing MADNESS wait policies without rebuilding. The default
  // remains Busy; the environment override affects only this process.
  ...
  if (argc < 2) {
    std::cerr << "Usage: ta_sequant_native_residual_main <data_dir>\n"
              ...
              << "    MAD_NUM_THREADS=N  MADNESS application thread count. Tune it with\n"
              << "      affinity for the current host; do not reuse machine-specific campaign values.\n";
    return 1;
  }
